...
def check_certificate(ip, port):
    ...
    not_after_regex = re.compile(r'NotAfter:\s*(\w{3} \d{2} \d{2}:\d{2}:\d{2} \d{4} GMT)')
    not_before_regex = re.compile(r'NotBefore:\s*(\w{3} \d{2} \d{2}:\d{2}:\d{2} \d{4} GMT)')
    ...
    if "errno=60" in stderr_output:
        logger.error({"ip": ip, "port": port, "ERROR": "Connection timed out", "detail": stderr_output})
        return {'ip': ip, 'status': 'connection_timeout', 'service': 'SSL Certificate Check'}

    elif "unexpected eof" in stderr_output:
        pass

    elif process.returncode != 0:
        logger.error({"ip": ip, "port": port, "ERROR": "SSL connection failed", "detail": stderr_output})
    ...
    if not_before_match and not_after_match:
        ...
        try:
            ...
            logger.error({"ip": ip, "ERROR": "Failed to parse the dates", "exception": str(e)})
            return {'ip': ip, 'status': 'parse_error', 'service': 'SSL Certificate Check'}

        now = datetime.datetime.now()
        if not_after_date < now:
            return {'ip': ip, 'port': port, 'status': 'expired', 'service': 'SSL Certificate Check'}
        elif (not_after_date - now).days <= expire_threshold_days:
            return {'ip': ip, 'port': port, 'status': 'expiring_soon', 'service': 'SSL Certificate Check'}
        elif (now - not_before_date).days <= recent_threshold_days:
            return {'ip': ip, 'port': port, 'status': 'recently_issued', 'service': 'SSL Certificate Check'}
        else:
            return {'ip': ip, 'port': port, 'status': 'valid', 'service': 'SSL Certificate Check'}
    else:
        logger.error({"ip": ip, "ERROR": "Certificate dates not found"})
        return {'ip': ip, 'status': 'date_info_missing', 'service': 'SSL Certificate Check'}
